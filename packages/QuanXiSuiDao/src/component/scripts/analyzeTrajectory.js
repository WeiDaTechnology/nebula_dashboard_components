...
function analyzeTrajectory() {
  ...
  console.log(
    `场景坐标 X: ${meta.coordRange.x.min.toFixed(2)} ~ ${meta.coordRange.x.max.toFixed(2)} (跨度: ${xRange.toFixed(2)})`,
  )
  console.log(
    `场景坐标 Y: ${meta.coordRange.y.min.toFixed(2)} ~ ${meta.coordRange.y.max.toFixed(2)} (跨度: ${yRange.toFixed(2)})`,
  )
  ...
  let totalDeltaX = 0
  let totalDeltaY = 0
  const vehicleDirections = { NE: 0, NW: 0, SE: 0, SW: 0, N: 0, S: 0, E: 0, W: 0 }
  ...
  Object.entries(angleHistogram)
    ...
    .forEach(([angle, count]) => {
      ...
      console.log(
        `  ${angle.padStart(3)}°-${(Number.parseInt(angle) + 30).toString().padStart(3)}°: ${bar} (${((count / totalAngles) * 100).toFixed(1)}%)`,
      )
    })

  // 分析每辆车的轨迹长度
  console.log('\n=== 轨迹长度分析 ===')
  const trajectoryLengths = vehicles
    .map(v => {
      let length = 0
  ...
  console.log(
    `最长轨迹: 车辆 ${trajectoryLengths[0].id}, 长度 ${trajectoryLengths[0].length.toFixed(2)}, ${trajectoryLengths[0].points} 个点`,
  )
  console.log(
    `最短轨迹: 车辆 ${trajectoryLengths[trajectoryLengths.length - 1].id}, 长度 ${trajectoryLengths[trajectoryLengths.length - 1].length.toFixed(2)}, ${trajectoryLengths[trajectoryLengths.length - 1].points} 个点`,
  )
}
